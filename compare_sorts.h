 ...
 * PROMISES: returns true if words in in the array are unique. Otherwise, returns false.
 */
bool is_unique(char words[MAX_UNIQUE_WORDS][MAX_WORD_SIZE], int num_words, const char *word);

/* REQUIRES: left > 0, right > 0
 * PROMISES: Creates a partitioning index for the quick sort function
 */
...
/* REQUIRES: indices points to an array of integer, holding index numbers,
 * words refer to a 2-D array of character, and each row is a valid c-string terminated with a '\0'
